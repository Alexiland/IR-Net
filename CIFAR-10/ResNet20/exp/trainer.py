...
def main():
    ...
    bnn_model = torch.nn.DataParallel(resnet.__dict__[args.arch]())
    bnn_model.cuda()
    ...
    # this defines full precision pretrained optimizer and lr rate
    optimizer = torch.optim.SGD(model.parameters(), args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)

    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.pretrained_epochs, eta_min = 0, last_epoch=-1)
    
    # this defines BNN optimizer and lr rate
    bnn_optimizer = torch.optim.SGD(bnn_model.parameters(), args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)

    bnn_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(bnn_optimizer, args.epochs, eta_min = 0, last_epoch=-1)
    ...
    # if args.pretrained:
        ...
        best_pretrained_file = os.path.join(args.save_dir, 'pretrained_best_model.pth.tar')
        save_checkpoint({
            'state_dict': model.state_dict(),
            'best_prec1': best_prec1,
    ...
    if args.pretrained:
        if os.path.isfile(best_pretrained_file):
            print("=> BNN loading pretrained checkpoint '{}'".format(best_pretrained_file))
            checkpoint = torch.load(best_pretrained_file)
            best_prec1 = checkpoint['best_prec1']
            bnn_model.load_state_dict(checkpoint['state_dict'])
            print("=> BNN loaded pretrained checkpoint best_prec1:'{}' "
                  .format(best_prec1))
        else:
            print("=> no checkpoint found at '{}'".format(best_pretrained_file))
    for epoch in range(args.pretrained_epochs, args.pretrained_epochs+args.epochs):
        ...
        for i in range(3):
            ...
            bnn_model.module.layer1[i].conv1.k = k
            bnn_model.module.layer1[i].conv2.k = k
            bnn_model.module.layer1[i].conv1.t = t
            bnn_model.module.layer1[i].conv2.t = t

            bnn_model.module.layer2[i].conv1.k = k
            bnn_model.module.layer2[i].conv2.k = k
            bnn_model.module.layer2[i].conv1.t = t
            bnn_model.module.layer2[i].conv2.t = t

            bnn_model.module.layer3[i].conv1.k = k
            bnn_model.module.layer3[i].conv2.k = k
            bnn_model.module.layer3[i].conv1.t = t
            bnn_model.module.layer3[i].conv2.t = t

        # train for one epoch
        print('current lr {:.5e}'.format(bnn_optimizer.param_groups[0]['lr']))
        train(train_loader, bnn_model, criterion, bnn_optimizer, epoch, is_pretrained=False)
        bnn_lr_scheduler.step()

        # evaluate on validation set
        prec2 = validate(val_loader, bnn_model, criterion, is_pretrained=False)
        ...
        if epoch > 0 and epoch % args.save_every == 0:
            save_checkpoint({
                'epoch': epoch + 1,
                'state_dict': bnn_model.state_dict(),
